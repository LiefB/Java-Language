...
public class OutterSort {
    ...
    public static void sort(File inputFile, File outputFile, File tempFile) throws Exception {
        ...
        String[] heapArray = new String[maxSize];   //用于置换选择的maxSize条记录的数组(实现堆)
        int heapSize = 0;   //堆大小
        String line = null;    //用来存放每次从缓冲区读入的一条记录
        int i = 0;  //统计向缓冲区读入了记录条数
        List<File> tempFiles = new ArrayList<>();   //所有的顺串

//<1> replaceSelection begin
        while ((line = bufr.readLine()) != null) {  //inputfile数据没有读完
            heapArray[i++] = line;
            if (i == maxSize)
                break;
        }
        while (i == maxSize) {
            ...
            buildHeap(heapArray, heapSize, 0);  //初始建堆
            while (heapSize != 0) {
                ...
                if (line == null)
                    break;
                if (keyOf(line).compareTo(keyOf(heapArray[0])) > 0) {   //读入的新记录 > heapArray[0]
                    heapArray[0] = line;
                } else {    //读入的新纪录 < heapArray[0]
            ...
            if (heapSize != 0) {    //case：file input is completed
                i = i - heapSize;

                //heapArray[0]已经已经write过了处理掉
                heapArray[0] = heapArray[heapSize - 1];
                heapSize--;
                siftDown(heapArray, 0, heapSize);
                //heapArray[0]已经已经write过了，处理掉
                while (heapSize != 0) {
                    bufw.write(heapArray[0]);
                    bufw.newLine();
                    heapArray[0] = heapArray[heapSize - 1];
...
//</1> replaceSelection end,all data are sorted into some separate temFile,all temFile are in tempFiles list.

        //release memory
        heapArray = null;
        System.gc();

//<2> begin MultiWayMergeSort
    ...
    static void multiWayMergeSort(List<File> files, File outputFile) throws IOException {
        int ways = files.size();    //路数 = 顺串数
        int length_per_run = maxSize / ways;    //每路中加载的记录条数
